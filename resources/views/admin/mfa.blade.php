...
<head>
    ...
    <title>MFA — {{ config('app.name', 'otshare.io') }}</title>
    ...
    <div class="relative mx-auto flex min-h-full max-w-md flex-col justify-center px-4 py-12 sm:px-6">
        <div class="admin-panel p-8">
            ...
            <p class="mt-2 text-sm leading-relaxed text-slate-500">
                Enter the 6-digit code from your authenticator app. While this browser stays open, you stay signed in for up to
                <strong class="text-slate-400">{{ $mfaSessionHours }} hours</strong> after each successful check. Closing the browser clears the admin session.
            </p>
            ...
            <form method="post" action="{{ route('admin.mfa.verify') }}" class="mt-6 space-y-4" data-admin-validate="totp">
                @csrf
                <input type="hidden" name="key" value="{{ $key }}">
                <div>
                    <label for="mfa-code" class="mb-1.5 block text-[10px] font-semibold uppercase tracking-wider text-slate-500">One-time code</label>
                    <input
                        id="mfa-code"
                        name="code"
                        type="text"
                ...
                <button type="submit" data-admin-submit class="relative inline-flex min-h-[2.75rem] w-full items-center justify-center rounded-xl bg-sky-600 py-3 text-sm font-semibold text-white shadow-lg shadow-sky-900/30 transition hover:bg-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-400/50 disabled:cursor-not-allowed disabled:opacity-40">
                    <span class="admin-btn-label">Verify &amp; continue</span>
        ...
        <p class="mt-6 text-center text-xs text-slate-600">TOTP authenticator only (no SMS or email OTP).</p>
    </div>
</body>
</html>
